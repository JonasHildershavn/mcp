/**
 * Enhanced Server Panel with Support for Tools, Resources, and Prompts
 *
...
interface ServerPanelEnhancedProps {
  serverName: ServerType;
}

type TabType = 'tools' | 'resources' | 'prompts';

export function ServerPanelEnhanced({ serverName }: ServerPanelEnhancedProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (toolsList.length > 0) setActiveTab('tools');
      else if (resourcesList.length > 0) setActiveTab('resources');
      else if (promptsList.length > 0) setActiveTab('prompts');
